...
class contactus(http.Controller):
    ...
    @http.route(['/page/website.issue', '/page/issue'], type='http', auth="public", website=True)
    ...
    def create_issue(self, request, values, kwargs):
        ...
        return request.registry['project.issue'].create(request.cr, SUPERUSER_ID, values, request.context)
    ...
    @http.route(['/project/issue'], type='http', auth="public", website=True)
    def contactus(self, **kwargs):
        ...
        for field_name, field_value in kwargs.items():
            ...
            elif field_name in request.registry['project.issue']._all_columns and field_name not in _BLACKLIST:
        ...
        if post_description:
            values[
                'description'] += dict_to_str(_("Custom Fields: "), post_description)
        ...
        if kwargs.get("show_info"):
            ...
            post_description.append(
                "%s: %s" % ("ACCEPT_LANGUAGE", environ.get("HTTP_ACCEPT_LANGUAGE")))
            post_description.append(
                "%s: %s" % ("REFERER", environ.get("HTTP_REFERER")))
            values[
                'description'] += dict_to_str(_("Environ Fields: "), post_description)
        ...
        if issue_id:
            for field_value in post_file:
                ...
                request.registry['ir.attachment'].create(
                    request.cr, SUPERUSER_ID, attachment_value, context=request.context)

        values = self.preRenderThanks(request, values, kwargs)
        return request.website.render(kwargs.get("view_callback", "website_issue.issue_thanks"), values)
